...
public class Day21Part2 {
    ...
    protected Long solve(final List<String> input, final int level) {
        final Map<Character, Position> numericKeypadPositionMap = getPositionMap(numPad);
        final Map<Character, Position> directionalKeypadPositionMap = getPositionMap(dirPad);
        final Map<Move, String> numPadMoves = generateNumPadMoves(numericKeypadPositionMap);
        final Map<Move, String> dirPadMoves = generateDirectionalPadMoves(directionalKeypadPositionMap);
        final Map<Key, Long> cache = new HashMap<>();
        
        return input.parallelStream().map(code -> {
            final long codeNumber = getNumber(code);
            return codeNumber * getNrChars(code, numPadMoves, dirPadMoves, cache, 25);
        }).reduce(0L, Long::sum);
    }
}
